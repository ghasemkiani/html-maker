...
const bootstrap = {
  makeBootstrap0({ node, nhtml, nhead, nbody, rtl = false, onLoaded }) {
    let { x } = this;
  ...
  makeBootstrapHead({ node, nhtml, nhead, rtl = false, onLoaded }) {
    let { x } = this;
  ...
  makeBootstrapBody({ node, nbody, onLoaded }) {
    let { x } = this;
    let nscript;
    x.chain(node || nbody, (node) => {
      x.ch(
  ...
  makeBootstrap({ node, nhtml, nhead, nbody, rtl = false, onLoaded }) {
    let { x } = this;
    let { nlink, promise: promiseHead } = this.makeBootstrapHead({
      node,
      nhtml,
      nhead,
      rtl,
    });
    let { nscript, promise: promiseBody } = this.makeBootstrapBody({ node, nbody });
  ...
  makeBootstrapNavBarItemDropdownV5({ node, text, onDropdown, onItems }) {
    let maker = this;
    let { x } = maker;
    let ndropdown;
    let nitems;
  ...
  makeBootstrapDropdownItemV5({ node, uri, text, onLink }) {
    let maker = this;
    let { x } = maker;
    x.ch(node, "li", (node) => {
  ...
  makeBootstrapDropdownDividerV5({ node }) {
    let maker = this;
    let { x } = maker;
    x.ch(node, "li", (node) => {
  ...
  makeBootstrapPills({ node, items, fixed, active, makeRelativeUri = (uri) => uri }) {
    let { x } = this;
    let nodeNav;
  ...
  makeBootstrapAlert({ node, kind = "info", f }) {
    let { x } = this;
    let ndiv;
    x.chain(node, (node) => {
...
const dialog = {
  ...
  makeDialogClose({ node }) {
    let { x } = this;
    x.chain(node, (node) => {
      x.attr(node, "data-bs-dismiss", "modal");
    });
  },
  makeDialog({ node, id = "dialog", title, onTitle, onHeader, onBody, onFooter }) {
    let { x } = this;
    let nodeDiv;
